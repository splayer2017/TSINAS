...
impl Db {
    ...
    pub fn open_in_memory() -> anyhow::Result<Self> {
        ...
        conn.execute_batch(
            "CREATE TABLE files(
                ...
                mime TEXT NOT NULL DEFAULT '', policy TEXT NOT NULL DEFAULT 'stream_only',
                host_id TEXT NOT NULL DEFAULT '');
             CREATE TABLE devices(
                endpoint_id TEXT PRIMARY KEY, tailnet_ip TEXT NOT NULL DEFAULT '',
                name TEXT NOT NULL DEFAULT '', last_seen INTEGER NOT NULL DEFAULT 0);",
        )?;
        Ok(Self {
            inner: Arc::new(Mutex::new(conn)),
        })
    ...
    pub fn upsert_file(&self, row: &FileRow) -> anyhow::Result<()> {
        ...
        conn.execute(
            "INSERT INTO files(path,hash,size,mime,policy,host_id)
             VALUES(?,?,?,?,?,?)
    ...
    pub fn list_files(&self) -> anyhow::Result<Vec<FileRow>> {
        let conn = self.inner.lock().unwrap();
        let mut stmt =
            conn.prepare("SELECT path,hash,size,mime,policy,host_id FROM files ORDER BY path")?;
        let rows = stmt
            .query_map([], |r| {
    ...
    pub fn get_by_hash(&self, hash: &str) -> anyhow::Result<Option<FileRow>> {
        let conn = self.inner.lock().unwrap();
        let mut stmt =
            conn.prepare("SELECT path,hash,size,mime,policy,host_id FROM files WHERE hash=?")?;
        let mut rows = stmt.query_map([hash], |r| {
            let policy_s: String = r.get(4)?;
            Ok(FileRow {
                path: r.get(0)?,
                hash: r.get(1)?,
